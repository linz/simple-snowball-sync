...
import { endpoint, target, verbose } from './common.js';
...
export const commandValidate = command({
  ...
  handler: (args) => {
    return Tracer.startRootSpan('command:validate', async (span) => {
      ...
      for (const file of targetManifest.files.values()) {
        ...
        if (sizeMissMatchCount) span.setAttribute('sizeMissMatchCount', sizeMissMatchCount);
      }
      // Found extra files in the destination, not necessarily a problem
      if (extraFiles.size > 0) {
        span.setAttribute('extraFilesCount', extraFiles.size);
      ...
      if (expectedFiles.size > 0) {
        span.setAttribute('missingFileCount', expectedFiles.size);
