...
async function findMostRecentUpload(mani: Manifest, bucket: string, prefix: string[], checkRange: number): Promise<number> {
  let foundUploaded = -1;
  ...
  while (low <= high) {
    ...
    logger.info({ index: mid, path: ctx.Key, exists: found != false, foundUploaded, foundNotUploaded, low, high }, 'LastUpload:Search');
    count++;
    if (count > 10) break;
  }
  for (let i = foundUploaded - checkRange; i < foundNotUploaded + checkRange; i++) {
    ...
    logger.debug({ index: i, path: ctx.Key, exists: head != true, sizeS3: head && head.ContentLength, sizeLocal: file.size }, 'LastUpload:SearchForComplete');
  ...
  async run(): Promise<void> {
    ...
    for (let index = startIndex; index < mani.files.length; index ++) {
      ...
      const p = queue(async () => {
        ...
        logger.debug({ index, path: file.path, size: file.size, target: `s3://${bucket}/${uploadCtx.Key}` }, 'Upload:Start')
